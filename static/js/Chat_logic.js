...
async function cambiarConversacion(id) {
    ...
    try {
        const resp = await fetch(`/api/chatbot/conversacion/${id}`);
...
async function crearNuevaConversacion() {
    ...
    await fetch('/api/chatbot/conversacion', {
        method: 'POST',
...
async function obtenerRespuestaReal(pregunta) {
    try {
        const resp = await fetch('/api/chatbot/consulta', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
async function enviarMensaje() {
    ...
    if (!tieneNombre && yaHayUsuario) {
        ...
        try {
            await fetch(`/api/chatbot/conversacion/${conversacionActual}`, {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json' },
...
async function verHistorialUsuario() {
    try {
        const resp = await fetch(`/api/chatbot/historial/${usuarioId}`);
        const data = await resp.json();
...
function guardarMensajeEnConversacion(mensaje) {
    ...
    fetch('/api/chatbot/conversacion/mensaje', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
/**
 * Registra la respuesta del usuario a una solicitud de feedback. (implementacion futura de modal dinamico)
...
async function cargarHistorialConversaciones() {
    try {
        const resp = await fetch(`/api/chatbot/conversaciones?usuarioId=${usuarioId}`);
        const data = await resp.json();
        ...
        await Promise.all(
            data.map(async (conv) => {
                try {
                    const r = await fetch(`/api/chatbot/conversacion/${conv.sessionId}`);
...
async function eliminarConversacion() {
    ...
    if (confirm('¿Eliminar esta conversación?')) {
        try {
            await fetch(`/api/chatbot/conversacion/${conversacionActual}`, { method: 'DELETE' });
